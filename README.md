...
The application includes a robust Web Application Firewall (WAF) based on Nginx acting as a reverse proxy with security headers and rate limiting, providing protection against common web attacks.

### WAF Features:
- Reverse proxy protection separating clients from backend services
- TLS/SSL encryption for all traffic
- Rate limiting to prevent brute force and DoS attacks
- Security headers protection against XSS, clickjacking, and other vulnerabilities
- HTTP to HTTPS redirection
- Input validation and sanitization
- Path restriction to prevent traversal attacks

### WAF Management:
```bash
# Start application with WAF protection (default)
./start-app.sh

# Start application without WAF protection
./start-app.sh --no-waf

# Display WAF logs
./start-app.sh waf logs

# Check WAF status
./start-app.sh waf status

# Restart WAF
./start-app.sh waf restart

# Display custom WAF rules
./start-app.sh waf rules
```

### Testing WAF Protection:
The repository includes a penetration testing script to verify the WAF's effectiveness:
```bash
...
The test script attempts various attack vectors including:
...
When WAF is enabled, the following endpoints are protected:
- Frontend: http://localhost (redirects to https://localhost)
- API Gateway: http://localhost/api (redirects to https://localhost/api)

### To properly reset do:
docker-compose down && docker-compose build auth-service && docker-compose up -d

### Reconciliation***
Manual reconcilation can be triggered with this route
curl -X POST http://localhost:3007/api/v1/users/reconcile
 
## User Service API - Friend Request Routes
### Friend Request API

#### 1. Get Friend Requests
```bash
curl -X GET http://localhost:8000/api/v1/users/3/friend-requests

# Response (when friend requests exist):
[{"id":4,"userId":1,"displayName":"Test User","avatarUrl":"data:image/jpeg;base64,SGVsbG8sIHRoaXMgaXMgYSB0ZXN0IGltYWdlIQ=="}]

# Response (when no friend requests):
[]
```

#### 2. Get Potential Friends
```bash
curl -X GET http://localhost:8000/api/v1/users/2/potential-friends

# Response:
[{"id":11,"displayName":"API Only Test","avatarUrl":null},{"id":3,"displayName":"testuser","avatarUrl":"/image/avatar/3df.jpeg"}]
```

#### 3. Send Friend Request
```bash
# Requires authentication
curl -X POST http://localhost:8000/api/v1/users/friend-requests \
  -H "Content-Type: application/json" \
  -d '{"senderId":2,"receiverId":4}'

# Error response when users are already friends:
{"error":"Already friends"}

# Error response when authentication is required:
{"error":"Failed to create friend request"}

# Success response (when authenticated and request succeeds):
{"id":5,"senderId":2,"receiverId":11,"status":"pending"}
```

#### 4. Accept Friend Request
```bash
curl -X POST http://localhost:8000/api/v1/users/friend-requests/4/accept

# Response:
{"success":true}
```

#### 5. Delete/Reject Friend Request
```bash
curl -X DELETE http://localhost:8000/api/v1/users/friend-requests/3

# Response:
{"success":true}
```

#### 6. Get User's Friends with Status
```bash
curl -X GET http://localhost:8000/api/v1/users/55/friends

# Response (when friends exist):
[
  {
    "id": 1,
    "userId": 123,
    ...
    "avatarUrl": "/image/avatar/123.jpg",
    "onlineStatus": "online"
  }
]

# Response (when no friends):
[]

 
```

#### 7. Get User's Online Status
```bash
curl -X GET http://localhost:8000/api/v1/users/55/status

# Response:
{"userId":55,"status":"offline"}
```

#### 8. Update User's Online Status
```bash
# Requires authentication as the user
curl -X PUT http://localhost:8000/api/v1/users/55/status \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"status": "online"}'

# Response:
{"id":1,"userId":55,"status":"online"}

# Possible status values: "online", "offline", "in_game", "away"

# Error when trying to update another user's status:
{"error":"Cannot update another user's status"}
```
...
# Response (when conversations exist):
[{"id":1,"otherUserId":3,"otherUsername":"user3","lastMessage":null,"lastMessageTime":"2025-05-01 3:57:11","unreadCount":0}]

# Response (when no conversations):
[]
```
...
# Response:
{"id":1,"participant1Id":10,"participant2Id":3,"createdAt":"2025-05-01 3:57:11"}

# Error response when trying to message yourself:
...
```bash
# Requires authentication
curl -X GET http://localhost:8000/api/v1/messages/conversations/user/3 \
  -H "Authorization: Bearer <token>"

# Response:
{"id":1,"participant1Id":10,"participant2Id":3,"createdAt":"2025-05-01 3:57:11"}

# Error response when conversation doesn't exist:
null
```
...
```bash
# Requires authentication
curl -X POST http://localhost:8000/api/v1/messages/conversations/1/messages \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"content": "Hello, this is a test message!"}'

# Response (successful):
{"id":1,"senderId":10,"receiverId":3,"content":"Hello, this is a test message!","read":false,"createdAt":"2025-05-01 3:57:11"}

# Error response when not a participant in conversation:
...
```bash
# Requires authentication
curl -X GET http://localhost:8000/api/v1/messages/conversations/1/messages \
  -H "Authorization: Bearer <token>"

# Response (when messages exist):
[{"id":2,"senderId":10,"receiverId":3,"content":"First test message","read":false,"createdAt":"2025-04-30 20:56:59"},{"id":3,"senderId":10,"receiverId":3,"content":"Second test message","read":false,"createdAt":"2025-05-01 3:57:11"}]

# Response (when no messages):
[]

# With pagination (limit):
curl -X GET "http://localhost:8000/api/v1/messages/conversations/1/messages?limit=1" \
  -H "Authorization: Bearer <token>"

# With pagination (before timestamp):
curl -X GET "http://localhost:8000/api/v1/messages/conversations/1/messages?before=2025-04-30%2020:57:11" \
  -H "Authorization: Bearer <token>"
```
...
```bash
# Requires authentication
curl -X PUT http://localhost:8000/api/v1/messages/conversations/1/read \
  -H "Authorization: Bearer <token>"
...
```bash
# Requires authentication
curl -X DELETE http://localhost:8000/api/v1/messages/messages/1 \
  -H "Authorization: Bearer <token>"

# Response:
{"success":true}

# Error response when message doesn't exist or you're not the sender:
{"error":"Message not found or you're not the sender"}
```
...
## Bot User Migration (Manual Step)
After initial DB setup and before using the system, run the following commands to create the three bot users (easy, medium, hard) in both auth-service and user-service:

```sh
docker compose exec auth-service node create-bots.js
docker compose exec user-service node create-bots.js
```

Alternatively, it should be executed upon invokation of start-app.sh. Thus you can 
throw aforementioned step out of the window.


## Monitoring System

The application includes a comprehensive monitoring system using Prometheus and Grafana. This section provides useful Prometheus queries to help you monitor the system.
...
```promql
# Check which services are up (1) or down (0)
up

# Filter to see status of a specific service
up{job="auth-service"}

# Count the number of services currently up
count(up{} == 1)
```
...
# Request rate for auth-service only
sum(rate(http_requests_total{job="auth-service"}[5m]))

# Request rate by status code
sum by(status) (rate(http_requests_total[5m]))

# Filter for error requests (4xx and 5xx)
...
histogram_quantile(0.95, sum(rate(http_request_duration_seconds_bucket{job="auth-service"}[5m])) by (le))

# Median (50th percentile) response time
histogram_quantile(0.50, sum(rate(http_request_duration_seconds_bucket[5m])) by (le))

# Compare response times across different endpoints
...
```promql
# CPU usage (if node-exporter is configured)
...
# Count of firing alerts by severity
...
1. `test-monitoring.sh`: Generates traffic with different types of requests (200s, 404s, etc.)
2. `test-alerts.sh`: Simulates a service outage to test alert functionality

Run these scripts to generate metrics data and test alert configurations.
...
colorboxtreasurebgSolidity smart contracts. These contracts will be responsi-
ble for recording, managing, and retrieving tournament scores.
```
```
...
```
Be careful, the management of duplicate usernames/emails is at your
...
- **Major module** : Multiple players
    ...
    (so the "remote players" module is strongly recommanded). It's up to you to decide
